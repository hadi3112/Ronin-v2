...
/**
 * PhaserBlocksPanel — replaces the IDE Editor panel in "Walk me through it" mode.
 * Loads Phaser and displays draggable code block tiles.
 * 
 * @param {{
 *   problemId: string;
 *   blocks: Array<{ id: string; code: string; explanation: string; correctPosition: number }>;
 *   onOrderChange: (newOrder: Array<{ id: string; correctPosition: number }>) => void;
 *   onDropdownOpened: (blockId: string) => void;
 * }} props
 */
  ...
  useEffect(() => {
    ...
    class TrainingBlocksScene extends Phaser.Scene {
      ...
      create() {
        ...
        // 3. Trigger initial shuffled order callback to React immediately on mount
