/**
 * PURPOSE: Renders the primary Training Grounds IDE workspace. It manages active states, provides a mode selection modal (Monaco Editor 'ide' vs Phaser Blocks 'blocks'), hooks into Pyodide WASM evaluations, controls the step-by-step verified submission overlay with custom lock states, gathers performance telemetry, and triggers intermediate progress dialogs.
 * DEPENDENCIES: 
 *   - React (v19)
 *   - React Router DOM (useParams, useNavigate, Link)
 *   - Framer Motion (motion, AnimatePresence)
 *   - Lucide Icons (ChevronLeft, Code2, X, Play, Send, Sparkles, CheckCircle2, XCircle, HelpCircle)
 *   - React Markdown
 *   - IDEQuestionPanel, IDEEditorPanel, IDEConsolePanel, IDEGeminiPanel, IDEBottomBar, PhaserBlocksPanel, LearnMoreDialog
 *   - executors/pythonExecutor.js (runPythonCode)
 *   - services/questionLoader.js (getQuestion)
 *   - services/trainingGroundsService.js (writeTrainingGroundsResult, computeAndSaveSkillVector)
 * USAGE CONTEXT: Main route page for the training sequences (Two Sum, Linked List Reversal, DFS Traversal, Circular Queue). Manages user progression, hint timers, and telemetry uploads.
 */
...
export default function IDETrainingPage() {
  ...
  const handleConfirmSubmit = () => {
    ...
    // Write telemetry payload to LocalStorage Firebase stubs
